...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_top_k_radix(ggml_metal_library_t lib, const char * variant) {
    char name[256];
    snprintf(name, sizeof(name), "kernel_top_k_radix_%s", variant);
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_dsv4_state_compress(ggml_metal_library_t lib, const ggml_tensor * op) {
    GGML_ASSERT(op->src[0]->type == GGML_TYPE_F32);
    GGML_ASSERT(op->src[1]->type == GGML_TYPE_F32);
    GGML_ASSERT(op->src[2]->type == GGML_TYPE_I32);
    GGML_ASSERT(op->type == GGML_TYPE_F32);

    const char * name = "kernel_dsv4_state_compress";
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_flash_attn_ext_vec(
    ...
    snprintf(name, 256, "%s_mask=%d_sink=%d_bias=%d_scap=%d_kvpad=%d_ns10=%d_ns20=%d_nsg=%d_nwg=%d_nq=%d",
            base,
            has_mask,
            has_sinks,
            has_bias,
    ...
    if (!res.pipeline) {
        ...
        if (nq > 1) {
            ggml_metal_cv_set_int32(cv, nq, FC_FLASH_ATTN_EXT_VEC + 24);
        }
