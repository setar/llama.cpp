...
void ggml_compute_forward_lightning_indexer(
    ...
    const bool mask_f16 = m->type == GGML_TYPE_F16;

    for (int s = 0; s < n_stream; ++s) {
        for (int t = 0; t < n_tokens; ++t) {
            const float * w_row =       (const float *) ((const char *)   w->data + t*nbw1 +        s*nbw3);
            const char  * m_row =                       ((const char *)   m->data + t*nbm1 + (s%nem3)*nbm3);
            float       * dst_row =           (float *) ((char *)       dst->data + t*nb1  +        s*nb3 );
            for (int ik = ir0; ik < ir1; ++ik) {
                const char * k_row = (const char *) k->data + ik*nbk2 + s*nbk3;
                const float * k_f32;
                ...
                for (int h = 0; h < n_head; ++h) {
                    ...
                    ggml_vec_dot_f32(n_embd, &qk, 0, q_row, 0, k_f32, 0, 1);
                    // ReLU and weights (prescaled)
                    score += MAX(qk, 0.0f) * w_row[h];
                }
                // apply mask
                const float mv = mask_f16 ? GGML_CPU_FP16_TO_FP32(((const ggml_fp16_t *) m_row)[ik])
                                          : ((const float *) m_row)[ik];
                dst_row[ik] = score + mv;
            }
        }
    }
...
// (duplicate ggml_compute_forward_lightning_indexer removed)
